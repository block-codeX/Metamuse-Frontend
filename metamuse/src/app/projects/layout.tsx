...
export default function AuthLayout({
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (searchRef.current && !searchRef.current.contains(event.target as Node) && !showSearchInput) {
    ...
    <div className="flex flex-col items-center justify-center h-screen bg-background">
      <header className="fixed top-0 h-[60px] w-full border-b border-gray-200 flex flex-row justify-between items-center px-4 bg-background z-10">
        <div className="flex flex-row space-x-4">
          ...
          <Button className="rounded-[40px] cursor-pointer bg-btn-primary dark:bg-btn-primary">
            Studio
          </Button>
      ...
      <div className=" fixed bottom-0 border border-red-600  flex flex-col justify-start items-start h-[calc(100vh-60px)] bg-background w-[100vw]  overflow-scroll">
        {children}
      </div>
    </div>
  );
