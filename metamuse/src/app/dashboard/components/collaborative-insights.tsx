...
import { api, humanizeDate } from "@/lib/utils";
...
export default function CollaborationInsights() {
  const [collabRequest, setCollabRequests] = useState([]);
  const fetchRequests = async () => {
    try {
      const apiInstance = api(true);
      console.log("API URL:", apiInstance.defaults.baseURL);
      const response = await api(true).get("/projects/invites/all");
      if (response.status === 200) {
        console.log(response.data)
        setCollabRequests(response.data.docs)
      }
      return false;
    } catch (error: any) {
      console.error(error)
    ...
    >
      ...
      <Card className="md:col-span-1">
        <CardHeader>
          ...
          <CardDescription>
            Solo vs. collaborative content creation
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 text-sm text-center text-muted-foreground">
            <p>
              Your collaborative works have increased by 42% in the last 3
              months
            </p>
          </div>
        </CardContent>
          ...
          {collabRequest.map((request) => (
            ...
            >
              ...
              <div className="flex-1">
                <h4 className="font-medium text-sm">{request.project.title}</h4>
                <p className="text-xs text-muted-foreground">
                  From: {request.collaborator.firstName} { request.collaborator.lastName}
                </p>
                <p className="text-xs text-muted-foreground">{humanizeDate(request.createdAt)}</p>
              </div>
