...
const OTPComponent = () => {
  ...
  const [otpData, setOtpData] = useLocalStorage<OtpData>("otp", {});
  ...
  const verifyUser = async (verificationToken: any) => {
    try {
      const response = await api().post("/auth/account/verify", {
        email: otpData.email || user?.email,
        otpData: { otpId: otpData.otpId, otpType: "EMAIL", verificationToken },
      });
      if (response.status == 201) {
        console.log(response.data);
        await fetchUser();
